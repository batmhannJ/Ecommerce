...
export const ListProduct = () => {
  ...
  const updateProduct = async () => {
    ...
    const response = await fetch('http://localhost:4000/editproduct', { method: 'POST', body: formDataToSend });
    if (response.ok) {
  ...
  const closeModal = (e) => { if (e.target.className === 'modal-overlay') setIsModalOpen(false); };
          ...
          <React.Fragment key={product.id}>
            <div className="listproduct-format">
              ...
              <div className="button">
                ...
                <button onClick={() => remove_product(product.id)} className='delete-button'>Delete</button>
              </div>
            </div>
            <hr />
          </React.Fragment>
          ...
          <div className="modal-content">
            <div className="modal-image-sidebar">
              <div className="image-upload-area">
                {formData.image ? (
                  <img src={typeof formData.image === 'string' ? formData.image : URL.createObjectURL(formData.image)} alt="Product" />
                ) : (
                  <span className="image-upload-placeholder">📷</span>
                )}
                <input type="file" accept="image/*" className="image-upload-input" onChange={handleFileChange} />
              </div>
            ...
            <div className="modal-form-section">
              ...
              <div className="form-grid">
                <div className="input-group">
                  ...
                  <input id="product-name" type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Product Name" autoFocus />
                </div>
                <div className="input-group">
                  <label htmlFor="old-price">Old Price</label>
                  <input id="old-price" type="number" value={formData.old_price} onChange={(e) => setFormData({ ...formData, old_price: e.target.value })} placeholder="Old Price" />
                </div>
                <div className="input-group">
                  <label htmlFor="new-price">New Price</label>
                  <input id="new-price" type="number" value={formData.new_price} onChange={(e) => setFormData({ ...formData, new_price: e.target.value })} placeholder="New Price" />
                </div>
                <div className="input-group">
                ...
                <div className="input-group">
                  ...
                  <input id="s-stock" type="number" value={formData.s_stock} onChange={(e) => setFormData({ ...formData, s_stock: e.target.value || 0 })} placeholder="Small Stock" />
                </div>
                <div className="input-group">
                  <label htmlFor="m-stock">Medium Stock</label>
                  <input id="m-stock" type="number" value={formData.m_stock} onChange={(e) => setFormData({ ...formData, m_stock: e.target.value || 0 })} placeholder="Medium Stock" />
                </div>
                <div className="input-group">
                  <label htmlFor="l-stock">Large Stock</label>
                  <input id="l-stock" type="number" value={formData.l_stock} onChange={(e) => setFormData({ ...formData, l_stock: e.target.value || 0 })} placeholder="Large Stock" />
                </div>
                <div className="input-group">
                  <label htmlFor="xl-stock">XL Stock</label>
                  <input id="xl-stock" type="number" value={formData.xl_stock} onChange={(e) => setFormData({ ...formData, xl_stock: e.target.value || 0 })} placeholder="XL Stock" />
                </div>
              </div>
              <div className="modal-buttons">
                <button className="update-button" onClick={updateProduct}>Update</button>
                <button className="cancel-button" onClick={() => { setEditProduct(null); setIsModalOpen(false); }}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
