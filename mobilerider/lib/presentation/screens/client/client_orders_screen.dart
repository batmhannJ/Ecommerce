    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<OrdersClient>>(
        ...
        builder: (context, snapshot) {
          print('FutureBuilder state: ${snapshot.connectionState}, hasData: ${snapshot.hasData}, error: ${snapshot.error}'); // Debug log
          ...
          if (snapshot.hasError) {
            return Center(
              child: TextCustom(text: 'Error: ${snapshot.error}', color: Colors.red),
            );
          }
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            print('No data or empty list: ${snapshot.data?.length ?? 'null'}'); // Debug log
            return Center(
              child: SvgPicture.asset('Assets/empty-cart.svg'),
            );
          }
          print('Rendering orders: ${snapshot.data!.length}'); // Debug log
...
class _ListOrdersClient extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    print('ListOrdersClient received: ${listOrders.length} orders'); // Debug log
    return (listOrders.length != 0)
        ? ListView.builder(
            ...
            itemBuilder: (context, i) => GestureDetector(
              onTap: () => Navigator.push(
                context,
                routeFrave(page: ClientDetailsOrderScreen(orderClient: listOrders[i])),
              ),
              child: Container(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        TextCustom(
                          text: '\$ ${listOrders[i].amount}0', // Fix formatting below
                          fontSize: 16,
                        ),
                      ],
                    ),
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        TextCustom(
                          text: DateCustom.getDateOrder(listOrders[i].currentDate.toString()),
                          fontSize: 15,
                        ),
                      ],
                    ),
        ...
        : Center(child: SvgPicture.asset('Assets/empty-cart.svg'));
  }
}